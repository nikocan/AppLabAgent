...
from .navigation import MenuItem, NavigationMenu, slugify
...
class AppLabAgentPlatform:
    ...
    # Frontend menü yapısını oluşturan ve eksikleri raporlayan metot.
    def navigation_menu(self) -> Mapping[str, object]:
        """Platformdaki kaynaklara hızlı erişim sağlayan menü özetini döndürür."""

        menu = NavigationMenu()

        projects = self.workspace.list_projects()
        accounts = self.accounts.list_accounts()
        automations = self.automation_studio.list_recipes()

        overview_section = menu.ensure_section(
            "overview",
            "Genel Bakış",
            "Önemli raporlar ve portföy istatistikleri için başlangıç noktası.",
        )
        overview_section.add_item(
            MenuItem(
                key="portfolio",
                label="Portföy Özeti",
                route="/portfolio",
                description="Projeleri, hesapları ve otomasyonları tek ekranda toplar.",
                badge=str(len(projects)),
            )
        )

        accounts_section = menu.ensure_section(
            "accounts",
            "Hesaplar",
            "Bağlanan servis hesaplarını ve meta verilerini listeler.",
        )
        for account in accounts:
            metadata_summary = ", ".join(
                f"{key}: {value}" for key, value in sorted(account.metadata.items())
            )
            description = metadata_summary or "Bu hesap için meta veri tanımlanmadı."
            badge = f"{len(account.tags)} etiket" if account.tags else None
            accounts_section.add_item(
                MenuItem(
                    key=f"{account.provider}:{account.name}",
                    label=f"{account.provider.title()} · {account.name}",
                    route=f"/accounts/{account.provider}/{account.name}",
                    description=description,
                    badge=badge,
                )
            )

        projects_section = menu.ensure_section(
            "projects",
            "Projeler",
            "Aktif projeleri açıklamaları ve ilerleme oranıyla gösterir.",
        )
        for project in projects:
            progress = project.task_board.progress_report()
            badge = f"{progress['done']}/{progress['total']}" if progress["total"] else "0/0"
            projects_section.add_item(
                MenuItem(
                    key=project.name,
                    label=project.name,
                    route=f"/projects/{slugify(project.name)}",
                    description=project.description or "Bu proje için açıklama girilmedi.",
                    badge=badge,
                )
            )

        automations_section = menu.ensure_section(
            "automations",
            "Otomasyonlar",
            "Çalışma alanındaki otomasyon tariflerini ve tetikleyicilerini listeler.",
        )
        for recipe in automations:
            description = recipe.description or f"Tetikleyici: {recipe.trigger}"
            badge = f"{len(recipe.actions)} adım"
            automations_section.add_item(
                MenuItem(
                    key=recipe.name,
                    label=recipe.name,
                    route=f"/automations/{slugify(recipe.name)}",
                    description=description,
                    badge=badge,
                )
            )
        ...
        return menu.audit(
            required_sections=("overview", "accounts", "projects", "automations")
        )
